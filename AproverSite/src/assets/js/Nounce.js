...
export default Nounce=draw2d.shape.layout.VerticalLayout.extend({
    ...
    init: function (attr) {
        ...
        this.nounce=JSON.parse(localStorage.getItem("Nounce"));
    ...
    addInOut: function () {
        ...
        for (let i = 0; i < this.nounce.length; i++) { 
            var labelNounce = new LabelRight({
                text: this.nounce[i],
                stroke: 0,
                radius: 0,
                bgColor: null,
            ...
            var output = labelNounce.createPort("output");
            ...
            output.setName("output_" + labelNounce.id);
            output.setColor("#04773b");
            output.setBackgroundColor("#04773b");
            output.setValue(this.nounce[i]);

            this.add(labelNounce);
